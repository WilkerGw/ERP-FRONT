...
import {
  ProdutoSchema,
  TProdutoSchema,
} from "@/lib/validators/produtoValidator";
...
function ProdutoForm({
  ...
  const form = useForm<TProdutoSchema>({
    ...
    // --- MUDANÇA NOS VALORES PADRÃO ---
    defaultValues: initialData || {
      codigo: "",
      nome: "",
      // Usamos números aqui para combinar com o tipo final após a transformação do Zod
      precoCusto: 0,
      precoVenda: 0,
      estoque: 0,
      tipo: undefined,
  ...
  useEffect(() => {
    // A lógica de reset também precisa ser consistente
    form.reset(
      initialData || {
        codigo: "",
        nome: "",
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (data: TProdutoSchema) =>
      initialData?._id
        ? api.put(`/produtos/${initialData._id}`, data)
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Lente de Contato">
                    Lente de Contato
                  </SelectItem>
                  <SelectItem value="Serviço/Conserto">
                    Serviço/Conserto
                  </SelectItem>
                </SelectContent>
              </Select>
...
function ProdutosPage() {
  ...
  const { data: produtos, isLoading } = useQuery<Produto[]>({
    ...
    queryFn: async () =>
      api
        .get("/produtos", { params: { search: searchTerm } })
        .then((res) => res.data),
  });
