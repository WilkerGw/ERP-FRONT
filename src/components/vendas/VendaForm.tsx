...
export const VendaForm = ({ initialData, vendaId }: VendaFormProps) => {
  ...
  // Efeitos de busca (sem alteração)
  useEffect(() => {
  ...
  const adicionarProduto = (produto: IProduto) => {
    append({ produtoId: produto._id, nome: produto.nome, quantidade: 1, valorUnitario: 0 });
    setBuscaProduto('');
    setProdutos([]);
  };
  ...
  const onSubmit = async (data: TVendaFormValidator) => {
    try {
      ...
      if (error instanceof AxiosError && error.response?.data?.message) {
        alert(`Falha ao processar a venda: ${error.response.data.message}`);
      } else {
        alert('Falha ao processar a venda. Verifique os dados e tente novamente.');
      }
    }
  };
